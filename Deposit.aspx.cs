...
public partial class Deposit : System.Web.UI.Page 
{
    ...
    private void LoadGrid()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("select Id, Amount, Date from Deposit order by Date DESC", con);
        con.Open();
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string path = ConfigurationManager.ConnectionStrings["connection"].ConnectionString;
            SqlConnection con = new SqlConnection(path);
            SqlCommand cmd = new SqlCommand("pursedeposit", con);
            ...
            LoadGrid();
            LoadBalance();
            string script = "<script>alert('myPurse credited successfully')</script>";
    ...
    protected void grdExisting_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        try
        {
            ...
            if (cmd.Equals("Del"))
            {
                string path = ConfigurationManager.ConnectionStrings["connection"].ConnectionString;
                SqlConnection con = new SqlConnection(path);
                SqlCommand DelCommand = new SqlCommand("Delete Deposit where Id = @Id", con);
            ...
            else if (cmd.Equals("Edt"))
            {
                string path = ConfigurationManager.ConnectionStrings["connection"].ConnectionString;
                SqlConnection con = new SqlConnection(path);
                SqlCommand GetCommand = new SqlCommand("select Id,Amount, Date from Deposit where Id = @Id", con);
    ...
    protected void btnSave_Click1(object sender, EventArgs e)
    {
        try
        {
            ...
            string path = ConfigurationManager.ConnectionStrings["connection"].ConnectionString;
            SqlConnection con = new SqlConnection(path);
            SqlCommand GetCommand = new SqlCommand("update Deposit set Amount = @Amount, Date = @Date where Id = @Id", con);
